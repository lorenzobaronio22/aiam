...
def _validate_email(value: str) -> None:
    ...
    try:
        ...
        raise ValueError("Email value must be a valid email address.") from error
...
ATTRIBUTE_TYPE_REGISTRY: list[AttributeTypeDefinition] = [
    ...
    AttributeTypeDefinition(
        ...
        name="Email",
        description=(
             "An electronic mail address for a member. Must be a valid email "
             "address when provided, and can be left blank."
         ),
        validate_value=_validate_email,
     ),
]
